...
def controladorRaton(evento, x, y, banderas, contexto):
    ...
    if evento == cv2.EVENT_MOUSEMOVE:
        contexto["x"] = x
        contexto["y"] = y
    elif evento == cv2.EVENT_LBUTTONDOWN: # LBUTTON, RBUTTON, MBUTTON
        contexto["puntos"].append((x, y))
    elif evento == cv2.EVENT_RBUTTONDOWN: # LBUTTON, RBUTTON, MBUTTON
        contexto["puntos"].pop()
